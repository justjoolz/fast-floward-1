...
// This transaction allows the signer to purchase all Kitty Items
// listed from the marketCollectionAddress

transaction(marketCollectionAddress: Address) {
    ...
    prepare(signer: AuthAccount) {
        ...
        if self.saleCollection.getIDs().length == 0 {
            panic("No items listed at this address")    
        }
    ...
    execute {
        // Loop through all the listed Kitty Items 
        for itemID in self.saleCollection.getIDs() {
