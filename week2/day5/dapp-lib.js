...
module.exports = class DappLib {
  ...
  static async kittyItemsTransferKittyItem(data) {
    ...
    let result = await Blockchain.post({
      ...
      {
        recipient: { value: data.recipient, type: t.Address },
        amount: { value: parseInt(data.withdrawID), type: t.UInt64 }
      }
    );
    return {
      type: DappLib.DAPP_RESULT_TX_HASH,
  ...
  // kittyItemsMarketBuyAllMarketItems
  // calls transactions/kittyitemsmarket/buy_all_market_items.cdc
  //
  // signer/proposer/authorizer: data.signer
  //
  static async kittyItemsMarketBuyAllMarketItems(data) {
    ...
    let result = await Blockchain.post({
      ...
      'kittyitemsmarket_buy_all_market_items',
      {
        marketCollectionAddress: { value: data.marketCollectionAddress, type: t.Address }
      }
    );
