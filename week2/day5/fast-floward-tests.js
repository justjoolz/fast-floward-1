...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            assert.equal(res1.result, 1, "Kitty Market Sale collection should have length = 1")

            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res2.result[0], 1, "Sale collection should have itemId: 1 at index 0")
        ...
        it(`buys all the kittyitems!`, async () => {
            ...
            await DappLib.kibbleMintTokens(testData3)
            ...
            try {
                await DappLib.kittyItemsMarketBuyAllMarketItems(testData4)
            } catch (e) {
                console.log(e)
            }
        })
